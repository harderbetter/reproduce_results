...
from .adpolc_eval_metrics_cls import *
from .adpolc_NN import *
from .adpolc_params_table import *
...
def prep(save, d_feature, lamb, dataset,
    ...
    exp_name = now.strftime("/%Y-%m-%d-%H-%M-%S-AdpOLC-" + dataset)
...
def adpolc(d_feature, lamb, tasks, data_path, dataset, save,
           K, val_batch_size, num_neighbors,
           eps,num_iterations):
    ...
    for t in range(1, T + 1):
        ...
        res.append(["Val-Task %s/%s: loss:%s; dbc:%s; acc:%s ;dp:%s; eop:%s; disc:%s; cons:%s; time:%s sec." % (
            t, T, np.round(loss.item(), 4), np.round(fair, 10), np.round(accuracy, 10), np.round(dp, 10),
            np.round(eop, 10),
            np.round(discrimination, 10), np.round(consistency, 10),
            np.round(cost_time, 4))])
